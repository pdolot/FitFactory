...
import com.example.fitfactory.data.models.request.ContractTerminationRequest
...
class ContractTerminationViewModel : BaseViewModel() {
    ...
    fun terminateContract(id: Long, token: String, amount: Double){

        rxDisposer.add(retrofitRepository.contractTermination(ContractTerminationRequest(id, token, amount))
            .subscribeBy(
                onSuccess = {
                    if (it.status){
    ...
    fun createToken(expiryDate: String, cvc: String, passId: Long, amount: Double){
        ...
        stripe.createCardToken(card, UUID.randomUUID().toString(), object :
            ...
            override fun onSuccess(result: Token) {
                val token = result.id
                terminateContract(passId, token, amount)
            }
