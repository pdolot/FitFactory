...
class SignInViewModel : ViewModel() {
                    ...
                    localStorage.getFacebookAccount()?.let {
                        ...
                        MaterialDialog(activity).show {
                            ...
                            positiveButton(text = "ZAPISZ"){
                                ...
                                signUp(user, AccountType.FACEBOOK)
                            }
                    ...
                    localStorage.getGoogleAccount(email)?.apply {
                        ...
                        MaterialDialog(activity).show {
                            ...
                            positiveButton(text = "ZAPISZ"){
                                ...
                                signUp(user, AccountType.GOOGLE)
                            }
                            negativeButton(text = "ANULUJ"){
                                callResult.postValue(CancelSignIn())
                                dismiss()
    ...
    private fun signUp(user: SignUpRequest, accountType: AccountType){
        retrofitRepository.signUp(user)
            .subscribeBy(
                onSuccess = {
                    if (it.status){
                        when(accountType){
                            AccountType.GOOGLE -> localStorage.saveGoogleAccount(user.username, user.password, user.email)
                            AccountType.FACEBOOK -> localStorage.saveFacebookAccount(user.username, user.password, user.email)
                        }
